...
WINDOW_TITLE: str = "pygame window 14"
FPS: int = 24
...
class SpriteShape:
    ...
    def __init__(self, color_name: str, shape_name: str="rectangle", border_width: int=5, inner_color_name: str="black") -> None:
    ...
    def set_shape(self, image: pygame.Surface) -> None:
        image_dimension = image.get_rect().size
        match self._shape_name:
            case "rectangle":
                if self._border_width != 0:
                    pygame.draw.rect(image, self.inner_color, ((0, 0), image_dimension), 0)
                pygame.draw.rect(image, self.color, ((0, 0), image_dimension), self._border_width)
            case "circle":
                if self._border_width != 0:
                    pygame.draw.circle(image, self.inner_color, (image_dimension[0] / 2, image_dimension[1] / 2), min(image_dimension[0], image_dimension[1]) / 2, 0)
                pygame.draw.circle(image, self.color,(image_dimension[0] / 2, image_dimension[1] / 2), min(image_dimension[0], image_dimension[1]) / 2, self._border_width)
...
class App:
    ...
    def __init_actors(self) -> None:
        ...
        player_sprite_shape: SpriteShape = SpriteShape("white", "circle", 0, "grey")
        actor_00_sprite_shape: SpriteShape = SpriteShape("blue", "rectangle", 5, "black")
        actor_01_sprite_shape: SpriteShape = SpriteShape("red", "rectangle", 5, "grey")
        ...
        ActorSpriteDrivenBySpeed(self.__screen, actor_00, actor_00_sprite_shape, [self.__actors_sprites])
        ActorSpriteDrivenByRandom(self.__screen, actor_01, actor_01_sprite_shape, [self.__actors_sprites])

...
WINDOW_TITLE: str = "pygame window 19"
FPS: int = 24
...
class App:
    ...
    def __init_actors(self) -> None:
        self.__init_player()
        self.__init_random_actor()
        self.__init_driven_actor()
        ...
        for _ in range(self.__number_of_actors_sprites):
            ...
            self.__init_actor(random_position)
    ...
    def __init_driven_actor(self) -> None:
    ...
    def __init_random_actor(self) -> None:
        self.__actors_random_sprites = pygame.sprite.Group()
        actor_01: Actor = Actor(pygame.Vector2(210, 160), pygame.Vector2(1, 1))
        actor_01_sprite_shape = SpriteShapeFactory.create_sprite_shape("rectangle", (64, 64), "black")
        actor_01_sprite = ActorSpriteDrivenByRandom(self.__screen, actor_01, actor_01_sprite_shape, [self.__actors_random_sprites])
        actor_01_image_png = pygame.image.load("ladybug.png").convert_alpha()
        actor_01_sprite.image.blit(actor_01_image_png,(0, 0))

    def __init_actor(self, position) -> None:
        random_speed: pygame.Vector2 = pygame.Vector2(randint(-1, 1), randint(-1, 1))
        actor: Actor = Actor(position, random_speed)
            ...
            case pygame.KEYDOWN:
                match event.key:
                    ...
                    case pygame.K_DOWN:
                        self.__actor_driven.speed = (0, 1)
                    case pygame.K_SPACE:
                        self.__init_actor(self.__actor_driven.position)
            case pygame.KEYUP:
                self.__actor_driven.speed = (0, 0)
            case pygame.QUIT:

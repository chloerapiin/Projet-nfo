...
WINDOW_TITLE: str = "pygame window 15"
FPS: int = 24


class Actor:
    # position and speed could be delegated to ActorSprite
    # class Actor can still be used to handle attributes not linked with graphical representation
    # such as age, energy, score, etc.
    _position: pygame.Vector2
    # dimension display is delegated to SpriteShape
...
class SpriteShapeFactory(ABC):
    @classmethod
    def create_sprite_shape(cls, shape_name: str, dimension: Tuple[int, int] | int, color_name: str,
                            border_width: int = 0, inner_color_name: str = "black") -> "SpriteShape":
        match shape_name:
            case "rectangle":
...
class SpriteShapeRectangle(SpriteShape):
    ...
    def __init__(self, dimension: Tuple[int, int], color_name: str, border_width: int = 5,
                 inner_color_name: str = "black") -> None:
...
class App:
    ...
    def __init_actors(self) -> None:
        ...
        random_speed: pygame.Vector2 = pygame.Vector2(randint(-1, 1), randint(-1, 1))
        actor_00: Actor = Actor(pygame.Vector2(randint(0, 420), randint(0, 320)), random_speed)
        actor_01: Actor = Actor(pygame.Vector2(210, 160), pygame.Vector2(0, 0))
        player_sprite_shape = SpriteShapeFactory.create_sprite_shape("circle", 60,"white", 0, "grey")
        actor_00_sprite_shape = SpriteShapeFactory.create_sprite_shape("rectangle", (40, 40), "blue", 5, "black")
